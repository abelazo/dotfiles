...
function github::get_prs {
  ...
  local repository=${2}
  local state=${3}
  local paginate_arg=${4}
  prs=$(gh api ${paginate_arg} -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" "/repos/${organization}/${repository}/pulls?state=${state}&per_page=100")
